...
- 解説文（data/text）は **未生成**。Claude Code CLI のログインが切れていたため（karteaude-login.bat で再ログイン後に `npm run data:text`）
- git 初期化・初回コミット済み。GitHub 組織 jap-ser ができたら remote を追加して push するだけ

## 康太さんにやってもらうこと（進捗）

- [ ] **karteaude-login.bat をダブルクリックして Claude Code に再ログイン**（解説文生成と、カルテのAI抽出の両方に必要）
